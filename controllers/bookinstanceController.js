    ...
    BookInstance.findById(req.params.id)
        ...
        .exec(function (err, bookinstance) {
            ...
            if (bookinstance == null) {
                var err = new Error('No book copies')
                err.status = 404
                return next(err)
            }
            //Successful, so render
...
exports.bookinstance_update_get = function (req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err); }
        res.render('bookinstance_form', { title: 'Update bookinstance', book_list: results.books, bookinstance: results.bookinstance, selected_book: results.bookinstance.book._id });
    })



    // res.render('bookinstance_form', { title: 'Update BookInstance', book_list: results.book, bookinstance: results.bookinstance });
};
